...
def transform_load_data(task_instance):
    data = task_instance.xcom_pull(task_ids="group_a.tsk_extract_chennai_weather_data")
...
def save_joined_data_s3(task_instance):
    ...
    df.to_csv(f"s3://weather-proj/{dt_string}.csv", index=False)
...
with DAG('weather_dag_2', ##Dag name should be unique
        default_args=default_args,
        schedule_interval = '@daily',
        catchup=False) as dag:

        start_pipeline = DummyOperator(
            task_id = 'tsk_start'   #should be unique throughout dag
        )
        ...
        with TaskGroup(group_id = 'group_a',tooltip= "Extractfrom_S3_and_weatherapi") as group_A:
            ...
            uploadS3_to_postgres  = PostgresOperator(
                ...
                postgres_conn_id = "postgres_conn",
                sql = "SELECT aws_s3.table_import_from_s3('city_look_up', '', '(format csv, DELIMITER '','', HEADER true)', 'weather-proj', 'ind_cities.csv', 'ap-south-1');"
            )
            ...
            is_chennai_weather_api_ready = HttpSensor(
                task_id ='tsk_is_chennai_weather_api_ready',
                http_conn_id='weathermap_api',
                endpoint='/data/2.5/weather?q=chennai&appid=16b9e3e0f476e91463c92eb3317277dd'
            )
            extract_chennai_weather_data = SimpleHttpOperator(
                task_id ='tsk_extract_chennai_weather_data',
                http_conn_id='weathermap_api',
                endpoint='/data/2.5/weather?q=chennai&appid=16b9e3e0f476e91463c92eb3317277dd',
                method = 'GET',
            ...
            transform_load_chennai_weather_data = PythonOperator(
                task_id= 'transform_load_chennai_weather_data',
            ...
            create_table1 >> truncate_table >> uploadS3_to_postgres
            create_table_2 >> is_chennai_weather_api_ready >> extract_chennai_weather_data >> transform_load_chennai_weather_data >> load_weather_data
